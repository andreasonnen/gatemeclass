...
message("Label key: ", ifelse(is.null(args$`data.label_key`), "not provided", args$`data.label_key`))
...
extract_from_tar <- function(tar_path, single_file = FALSE) {
  message("  Extracting: ", basename(tar_path))
  ...
  message("  Found ", length(csv_files), " file(s)")
...
load_label_key <- function(path) {
  ...
  message("  Loading label key: ", basename(path))
  ...
  if ("id_to_label" %in% names(parsed)) {
    ...
    message("  Loaded ", length(mapping), " label mappings")
    return(mapping)
  }
...
message("  Train matrix: ", nrow(train_dt), " cells x ", ncol(train_dt), " markers")
...
message("  Train labels: ", length(train_y), " values")
...
for (nm in test_sample_names) {
  message("  Test '", nm, "': ", nrow(test_list[[nm]]), " cells x ", ncol(test_list[[nm]]), " markers")
}
...
print(table(train_y, useNA = "ifany"))
...
if (n_unlabeled > 0) {
  message("\nRemoving ", n_unlabeled, " unlabeled cells from training set")
  train_dt <- train_dt[!unlabeled_mask, ]
  train_y <- train_y[!unlabeled_mask]
}

message("After filtering: ", nrow(train_dt), " cells")
...
message("\nRenaming ", n_markers, " markers to M1..M", n_markers)
...
# ---------------------------------------------------------------------
message("Transposing matrices (GateMeClass expects markers x cells)...")
...
message("Train matrix: ", nrow(train_m), " markers x ", ncol(train_m), " cells")

# ---------------------------------------------------------------------
# Run GateMeClass on each test sample
# ---------------------------------------------------------------------
message("\n=== Running GateMeClass_annotate ===")
...
for (test_name in test_sample_names) {
  message("\nProcessing test sample: ", test_name)
  test_m <- test_matrices[[test_name]]
  message("  Test matrix: ", nrow(test_m), " markers x ", ncol(test_m), " cells")
  ...
  message("  Predicted: ", length(pred_labels), " cells; ", 
          length(unique(pred_labels)), " unique labels")
...
for (test_name in test_sample_names) {
  ...
  # Use same filename pattern as test labels tar
  # Original might be like "dataset-data-2" so we keep that
  tmp_file <- file.path(tmp_pred_dir, paste0(test_name, ".predictions.csv.gz"))
  
  con <- gzfile(tmp_file, "wt")
  writeLines(out_labels, con)
  close(con)
  
  message("  Prepared: ", test_name, " (", length(pred_int), " predictions)")
}
...
message("\nCreating predictions archive: ", pred_archive)

old_wd <- getwd()
setwd(tmp_pred_dir)

# List files in sorted order (must match test labels order!)
files_to_tar <- sort(list.files(".", pattern = "\\.csv\\.gz$"))
message("  Files: ", paste(files_to_tar, collapse = ", "))
...
if (file.exists(pred_archive)) {
  finfo <- file.info(pred_archive)
  message("  Success! Archive size: ", finfo$size, " bytes")
} else {
